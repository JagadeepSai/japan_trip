...
## Iteration 50 — Vertical timeline redesign (2026-07-26)
**Ask:** "When vertical, it doesn't look that aesthetic and clean, check why" + follow-up "looks off still".
**Diagnosis (5-lens critique workflow, 36 findings):** the vertical mode was a mechanical transpose — phone-sized fixed 196px lanes hugging the left of a 1440px viewport (content ended at x≈288), empty time with no furniture (no gridlines/gap semantics/night treatment), the atmosphere layer rotated not redesigned (temp curve = unlabeled vermillion squiggle in a 34px rail; city tint a 3px sliver at the bezel), a debris left rail (4 unaligned columns), dock/sticky-chip collisions.
**Redesign:**
- Responsive centered column: every day centered in one content box (avail = min(boardW,760) − rail − margins); lanes side-by-side when cards stay ≥200px, else GCal-style cascade (offset ≥44px, z-index by lane). Desktop 1-lane = 340px cards at viewport center (750 vs 720); narrow 360px window = 193px+ cascade cards, no h-scroll.
- Full-bleed underlay behind the centered strip: re-tuned sky gradient (~2.5× alphas, multiply blend in day theme), per-day city bands at authored alpha, indigo midnight day-rules; city ribbon relocated to a 3px binding thread at each day's column edge.
- Instrument rail (64px/44px): right-aligned hour labels, ticks, warm-toned temp readouts at 09/15/21 (desktop), sunrise/sunset horizon lines + labels (collision-aware: flips below only just-after even hours). Vertical temp polyline + rain slivers deleted (unanimous critic verdict); rain = full-column gradient washes.
- Furniture: 2h gridlines across the column, free-time gap pills (dashed thread + duration) for ≥75min visual gaps.
- Cards: content leads left, portrait thumbs (44px; ≥120px-tall cards get clamp(44px,38%,88px) poster thumbs; cascade-slim cards get 40px squares), 3-line title clamp, ghost duration track + solid span, ◷ moved to meta line, hover translateX, passive de-ghosted, tighter shadow, z≥1 above furniture.
...
## Iteration 52 — Event costs in yen (2026-07-27)
**Ask:** cost field on events — multiple tiers/packages with labels, exactly one chosen ("active") like a package pick, edited in the event modal; active cost subtle on the hover card; DB support; research real costs for all events from official sites/web and update the DB; show the trip's active-cost total aesthetically in Plan.
**Model:** reserved meta key `costs` = JSON `[{label, yen, active}]` (per person, integer yen); one active enforced on read/save. Helpers wishCosts/wishActiveCost/costLine/formatYen.
**UI:** "Cost · 円" collapsible in the event modal — radio (chosen) + label + ¥ field rows, add/remove, live summary hint showing the chosen tier; auto-opens when the event has costs. Hover card (`hc-cost`) + wish preview get a muted 0.6–0.66rem line "¥3,000 · Tembo Deck + Galleria" (0 yen → "Free"). Plan header: 旅費 pill (kanji kicker, tabular yen, "/ person") between density toggles and trip dates; hidden at ¥0; night variant.
**Data:** 5 parallel research agents priced all 160 events (official sites first: Skytree ¥3,000 advance combo, Nijo ¥1,300, Himeji ¥2,500 (2026 rise), Hakone Freepass ¥7,100, teamLab date-priced, USJ ¥9,900, Gion Corner ¥5,500, Kiyomizu ¥500…; typical-spend estimates for meals/bars/ryokan flagged "estimate"; booked flights skipped). Validated (160/160, one active each, integer yen; 53 free, median ¥780) → PATCHed into live Supabase (158 rows) → seeds regenerated from DB (data.js + schema.sql). Live total: **¥143,580/person**.
**Verified:** hermetic — editor save/reopen/active-radio/live-hint, plan chip, preview+hovercard lines (10.56px muted), Free rendering, mobile fit (chip + rows); live-DB — chip totals ¥143,580 from real data. `?v=20260845a`.

## Iteration 53 — Plan table view (2026-07-27)
**Ask:** data-table view of all events in Plan, feature-rich, filters incl. all-vs-active (active default ON); brainstorm the columns.
**Columns (from the full event-field inventory):** Event (26px thumb / type-dot + name + ⓘ notes badge) · Type (colored-edge chip) · Day (D# · date, Inbox italic) · Time · Dur · Cost (chosen package, tier sublabel, tooltip = all tiers with ● marking the choice) · Place · City (day-derived, else nearest) · 結び group · Status (● On route / ◌ Maybe) · ✎ edit. Cut as noise: lat/lng, image URL, sort order; map/directions/chat live in the row-click preview.
**Features:** search (label/place/group), Type/Day(+Inbox)/City selects, "On-route only" chip (default ON — 78 rows vs 158 all), click-to-sort headers with ▲▼ (trip order default; cost/dur/time numeric), footer summary (count + chosen-package total for the filtered set), row click → preview, ✎ → edit modal, sticky header, internal h-scroll on mobile (page pinned), night theme, hidden 秘密 events follow the reveal toggle.
